...
export default function SeatSelection() {
  ...
  useEffect(() => {
    console.log('=== Travel Class Debug ===');
    console.log('state.searchParams.travelClass:', state.searchParams.travelClass);
    console.log('state.selectedTravelClass:', state.selectedTravelClass);
    ...
    if (state.searchParams.travelClass) {
      ...
      if (state.selectedTravelClass !== state.searchParams.travelClass) {
        ...
        dispatch({ type: 'SET_SELECTED_TRAVEL_CLASS', payload: state.searchParams.travelClass });
      }
    }
    ...
    <div className="max-w-6xl mx-auto">
      <div className="mb-8">
        ...
        <p className="text-gray-600 mt-2">
          {state.selectedFlight?.airline} Flight {state.selectedFlight?.flightNumber}
        </p>
        {/* Travel class display */}
        <div className="mt-2">
          <span className="inline-block bg-blue-100 text-blue-800 px-3 py-1 rounded-full text-sm font-medium">
            Travel Class: {state.selectedTravelClass ? state.selectedTravelClass.replace('_', ' ') : 'Not Set'}
          </span>
        </div>
      </div>
            ...
            <div className="bg-gray-50 rounded-lg p-6">
              ...
              <div className="border-t pt-4">
                <div className="flex justify-between mb-2">
                  ...
                  <span className="font-medium">
                    {state.selectedFlight?.price ? `₹${state.selectedFlight.price}` : '₹ 0'} × {state.selectedSeats.length}
                  </span>
                </div>
                <div className="flex justify-between mb-2">
              ...
              <button
                ...
                className={`w-full bg-primary text-white py-3 rounded-lg font-medium mt-6 hover:bg-blue-600 transition duration-300 ${
                  state.selectedSeats.length === 0 ? 'opacity-50 cursor-not-allowed' : ''
                }`}
              >
                Continue to Booking
              </button>
